<script lang="ts">
  import { useI18n, T } from '@comvi/svelte';

  const { t } = useI18n();

  let gender: 'male' | 'female' | 'other' = 'male';
  let formality: 'formal' | 'informal' = 'formal';
  let genderCount = 5;
  let formalityCount = 3;
  let nestedCount = 3;
</script>
...
<div class="space-y-8">
  ...
  <div class="space-y-2">
    ...
    <div class="p-4 bg-gray-50 rounded border">
      <T
        ...
        components={{
          link: { tag: 'a', props: { href: 'https://example.com', class: 'text-blue-600 hover:underline', target: '_blank' } }
        }}
      />
    </div>
  </div>
  ...
  <div class="space-y-2">
    ...
    <p class="text-sm text-gray-500 mt-2">
      Using the &lt;T&gt; component allows you to replace tags in translations with actual
      HTML elements securely.
    </p>
  </div>
  ...
  <div class="border-t pt-8 mt-8">
    ...
    <div class="space-y-3 mb-6">
      ...
      <p class="text-sm text-gray-500">
        Translation: <code class="bg-gray-100 px-1 rounded">&lt;link&gt;&lt;bold&gt;this link&lt;/bold&gt;&lt;/link&gt;</code>
    ...
    <div class="space-y-3 mb-6">
      ...
      <div class="flex items-center gap-4 mb-2">
        <label class="text-sm text-gray-600" for="nested-count-input">Count:</label>
        <input
          id="nested-count-input"
          type="number"
          min="0"
          bind:value={nestedCount}
      ...
      <div class="p-4 bg-yellow-50 rounded border border-yellow-200">
        <T
          ...
          components={{
            ...
            link: { tag: 'a', props: { href: '#', class: 'text-yellow-700 underline hover:text-yellow-900' } }
          }}
        />
      </div>
      <p class="text-sm text-gray-500">
        Translation: <code class="bg-gray-100 px-1 rounded">&lt;alert&gt;Warning: &lt;bold&gt;&#123;count&#125;&lt;/bold&gt; items...&lt;/alert&gt;</code>
      </p>
    </div>
    ...
    <div class="space-y-3 mb-6">
      ...
      <div class="p-4 bg-blue-50 rounded border border-blue-200">
        <T
          ...
          components={{
            ...
            header: { tag: 'div', props: { class: 'flex items-center gap-2 font-bold text-blue-900 mb-2' } },
      ...
      <p class="text-sm text-gray-500">
        Translation: <code class="bg-gray-100 px-1 rounded">&lt;card&gt;&lt;header&gt;&lt;icon/&gt;Title&lt;/header&gt;&lt;content&gt;...&lt;/content&gt;&lt;/card&gt;</code>
  ...
  <div class="border-t pt-8 mt-8">
    ...
    <div class="space-y-3 mb-6">
      ...
      <div class="flex items-center gap-4">
        <div class="flex gap-2">
          {#each ['male', 'female', 'other'] as g (g)}
            <button
              on:click={() => (gender = g as typeof gender)}
              class="px-3 py-1 rounded text-sm transition-colors {gender === g
    ...
    <div class="space-y-3 mb-6">
      ...
      <div class="flex items-center gap-4">
        <div class="flex gap-2">
          {#each ['formal', 'informal'] as f (f)}
            <button
              on:click={() => (formality = f as typeof formality)}
              class="px-3 py-1 rounded text-sm transition-colors {formality === f
                ? 'bg-blue-600 text-white'
                : 'bg-gray-200 hover:bg-gray-300'}"
            >
              {f}
