...
// built, so it composes none of the 0.4 batteries recipe itself — the graph
// still names core's base root, because that is where `createI18n` lives.
...
// the `next-server-on-slim` matrix case gates: the tag chunks never reach a
// server bundle.
