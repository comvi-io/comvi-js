...
// Both sibling packages are resolved through node_modules, not __dirname/..:
// inside a Stryker sandbox (.stryker-tmp copies only this package) relative
// paths out of the package do not exist, but the node_modules symlink chain
// still reaches the real workspace.
...
const REACT_SRC = resolve(
  dirname(createRequire(import.meta.url).resolve("@comvi/react")),
  "../src/index.ts",
);
