...
// dev-only tombstone.
//
// Solid idiom (§3.2): plain accessors under the Provider, NO signals. A
// capability action is an imperative operation, not a reactive value — the
// returned members are the bound host methods, and there is nothing to track.
// They read the context exactly like `useI18nContext()` does, so they are
// subject to the same rule: call them in a component/owner scope.
//
