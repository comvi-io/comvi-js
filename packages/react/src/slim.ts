// `@comvi/react/slim` — the single-package, star-free entry.
//
...
//      of its pure subpaths, never relayed through `@comvi/react`.
//
...
// `@comvi/core`, re-exported so a react app has one import specifier. There is
// no react-side wrapper object to build (the instance goes straight into
// `<I18nProvider i18n={…}>`), so a hand-written preset here would be a rename
// and nothing else.
export { createI18n } from "@comvi/core";

// The bindings — identical to `@comvi/react`. The only root value that entry
