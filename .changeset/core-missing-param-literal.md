...
Missing interpolation parameters now render as the literal placeholder instead of silently disappearing, governed by the new `missingParam` option (default `"literal"`).

Before: `t("greet")` with `greet: "Hello, {name}!"` produced `"Hello, !"`. Now it produces `"Hello, {name}!"`, plus one dev-mode warning per (template, parameter) pair. This aligns with ICU MessageFormat and every major i18n runtime, and makes missing data visible instead of corrupting copy.
