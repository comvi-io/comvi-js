...
Internal dedupe: the local `isVirtualNode` copy is replaced by core's own export (reached through `@comvi/core/slim`, so the binding does not pull in the root entry); `t`/`tRaw` types are now core's `TranslateFn` (inference unchanged). React's element-aware `translationResultToString` stays local by design.
