...
**The root re-exports `subscribeToRevision`, `REVISION_EVENTS` and `isVirtualNode`** (plus the `RevisionEvent` / `RevisionEventSource` types), so a binding that reads reactivity events or sniffs virtual nodes reaches them from the one entry — and, since that entry is side-effect-free, without pulling ambient tag registration into its bundle. Measured: the additions cost **0 B** — the then-base-host fixture stayed at its measured size and the minified byte count of every core and framework size fixture was unchanged.
