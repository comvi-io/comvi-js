...
State and input hardening — five fixes:
...
- `clearTranslations()` / `reloadTranslations()` cancel matching in-flight namespace loads, so stale responses can't repopulate a cleared cache and reload always fetches fresh data. Per-call `fallback` now skips the instance-level `onMissingKey` option (registered callbacks still fire).
