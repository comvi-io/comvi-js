import { expect, test, chromium, type Browser, type BrowserContext, type Page, type Worker } from "@playwright/test";
...
async function openPopup(
  ...
  const popup = connection.contexts().flatMap((context) => context.pages()).find((page) => page.url() === popupUrl);
...
async function activateEditor(
  ...
  try {
    ...
    throw new Error(`Editor activation timed out: ${JSON.stringify(diagnostics)}`, { cause: error });
  }
}

async function phaseFailures(page: Page, phase: string) {
  return page.evaluate((wantedPhase) => {
    const results = (globalThis as typeof globalThis & {
      __GATE_E_RESULTS__?: Array<{ phase: string; name: string; pass: boolean; detail?: string }>;
    }).__GATE_E_RESULTS__ ?? [];
...
test("built MV3 extension enforces the hostile-page trust boundary", async () => {
  ...
  try {
    ...
    const tabId = await worker.evaluate(() => {
      const chromeApi = (globalThis as typeof globalThis & { chrome: typeof chrome }).chrome;
      return chromeApi.tabs.query({ active: true, currentWindow: true }).then((tabs) => tabs[0]?.id);
    });
    ...
    await page.evaluate(() => { void (globalThis as any).gateE.runPhase5(); });
    ...
    await page.evaluate(() => { void (globalThis as any).gateE.runPhase4(); });
