...
// the svelte root exports, composed on one host from one specifier, plus <T>.
// ICU, loader, plugins, devtools and the pure rich-text path are all retained.
//
// RE-ANCHORED from `fw-svelte-root`, whose fixture measured the 0.4
// batteries-included era: back then an app named `@comvi/core` for a root that
// dragged composed core in whether the app used it or not. This row is no
// longer behavior-identical to that historical root because 0.4 also registered
// string-API tags ambiently; convergence deliberately makes that an explicit
// `@comvi/core/tags` import.
//
...
//
// Every capability exported by @comvi/svelte is otherwise present. `svelte` is
