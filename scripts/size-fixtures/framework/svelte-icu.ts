// Framework size fixture (single-entry P3): the default svelte app with ICU
// message formatting, still through ONE specifier. `icuCompiler` comes from
// `@comvi/svelte` — the same package as the constructor and the bindings —
// which is the whole point of the single-entry surface: reaching a capability
// never means reaching past your framework package.
//
// NEW row in P3, on the model of `react-icu.ts`. It exists because the root now
// re-exports the toolkit: an app that opts into exactly one capability must pay
// for exactly that one, and no row before this one measured that shape for
// svelte.
//
...
// The bundler-matrix `svelte-icu` case is where that presence is proved
