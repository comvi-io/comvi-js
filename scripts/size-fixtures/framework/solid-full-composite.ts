...
// the Solid root exports, composed on one host from one specifier, plus <T>.
// ICU, loader, plugins, devtools and the pure rich-text path are all retained.
//
// RE-ANCHORED from `fw-solid-root`, whose fixture measured the 0.4
// batteries-included era: back then the app's own `createI18n` named core's
// composed root, so the tag chunks stayed alive whatever <T> did, and the graph
// held BOTH core entries. This row is no longer behavior-identical to that
// historical shape because 0.4 also registered string-API tags ambiently;
...
//
// Every capability exported by @comvi/solid is otherwise present. `solid-js` is
