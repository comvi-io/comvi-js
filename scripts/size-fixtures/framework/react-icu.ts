// Framework size fixture (single-entry P2): the default react app with ICU
// message formatting, still through ONE specifier. `icuCompiler` comes from
// `@comvi/react` — the same package as the constructor and the bindings — which
// is the whole point of the single-entry surface: reaching a capability never
// means reaching past your framework package.
//
// NEW row in P2. It exists because the root now re-exports the toolkit: an app
// that opts into exactly one capability must pay for exactly that one, and no
// row before this one measured that shape for react.
//
...
// The bundler-matrix `react-icu` case is where that presence is proved
